...
lfi_proc_mapany(LFIProc* proc, size_t size, int prot, int flags, int fd, off_t offset)
{
    ...
    uintptr_t addr = mm_mapany(&proc->mm, size, prot, flags, (void*) (uintptr_t) fd, offset);
    if (addr == (uint64_t) -1)
        return NULL;
...
{
    uintptr_t addr = mm_mapat_cb(&proc->mm, start, size, prot, flags, (void*) (uintptr_t) fd, offset, cbunmap, NULL);
    if (addr == (uint64_t) -1)
        return (void*) -1;
    int r = procmap(proc, addr, size, prot, flags, fd, offset);
    if (r < 0) {
