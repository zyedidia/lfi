...
func checkMem(mem arm.Arg, inst Inst) {
	...
	case arm.MemExtend:
		...
		if arg.Extend != arm.Uxtw {
			fail(inst, "extend must be uxtw")
		}
	}
}
...
func main() {
	...
	for i := 0; i < len(insts); i++ {
		...
		switch inst.Op {
		case arm.MSR:
			if !isSystemReg(inst.Args[0], tpidr_el0) {
				fail(inst, "attempt to access non-tpidr_el0 system reg")
			}
		case arm.MRS:
			if !isSystemReg(inst.Args[1], tpidr_el0) {
				fail(inst, "attempt to access non-tpidr_el0 system reg")
			}
		case arm.BLR, arm.BR:
